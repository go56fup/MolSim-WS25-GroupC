...
public:
	constexpr explicit(false) flag_special_member_funcs(
...
protected:
	...
	static constexpr void runtime_log(std::string_view fmt_string, Args&&... args) {
		...
		// of a deriving class, a call to the baseclass' special member function is generated by the compiler.
		// This TRANSITIVELY forces immediate-promotion, making e.g. constexpr operator= for vec consteval,
		// and breaking runtime code.
		if not consteval {
			spdlog::trace(std::vformat(fmt_string, std::make_format_args(args...)));
		}
	}
};
