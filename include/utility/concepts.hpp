...
namespace detail {
// libc++ does not support this concept as of now (2025-11-29), so we copy the implementation from
// libstdc++.
...
using iter_const_reference_t =
	std::common_reference_t<const std::iter_value_t<It>&&, std::iter_reference_t<It>>;
...
concept constant_range =
	std::ranges::input_range<Tp> && constant_iterator<std::ranges::iterator_t<Tp>>;
#else
template <typename T>
...
concept range_of =
	...
	std::same_as<std::ranges::range_value_t<Range>, std::remove_const_t<Element>> &&
