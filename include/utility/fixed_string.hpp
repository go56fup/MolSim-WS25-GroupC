...
public:
	...
	template <std::size_t N2>
	[[nodiscard]] friend consteval bool
	operator==(const basic_fixed_string& lhs, const CharT (&rhs)[N2]) {
	...
	operator<<(std::basic_ostream<CharT>& os, const basic_fixed_string& str) {
		return os << str.c_str();
	}
...
// formatting support
template <typename CharT, std::size_t N>
struct fmt::formatter<p3094::basic_fixed_string<CharT, N>>
	: formatter<std::basic_string_view<CharT>> {
	template <typename FormatContext>
	auto format(const p3094::basic_fixed_string<CharT, N>& str, FormatContext& ctx) const
		-> decltype(ctx.out()) {
