...
struct json_data_contract<sim_configuration> {
	...
	using type = json_member_list<
		...
		json_string<"base_name">, json_class<"domain", vec>>;
};

template <>
struct json_data_contract<unprocessed_config> {
	using type = json_member_list<json_class<"configuration", sim_configuration>, json_raw<"bodies">>;
};
...
struct json_data_contract<disc_parameters> {
	using type = json_member_list<
		...
		json_class<"velocity", vec_2d<double>>, json_number<"brownian_mean", double>,
		json_number<"particle_mass", double>>;
};
